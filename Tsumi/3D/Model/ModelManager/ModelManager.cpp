...
/// Objファイルを読み込む
/// </summary>
ObjData ModelManager::LoadObjFile(std::string filePath) {
	...
	if (CheckObjData(filePath)) {
		...
		// ファイルを開く
		std::ifstream file("Resources/Obj/" + filePath + "/" + filePath + ".obj");
		// とりあえず開けなかったら止める
		assert(file.is_open());
		...
		while (std::getline(file, line)) {
			...
			if (identifier == "v") {
				...
				objData.material = ModelManager::Getinstance()->LoadMaterialTemplateFile(filePath, materialFileName);
			}
		}
		// テクスチャを指定されたものにする
...
/// mtlファイルを読み込む関数
/// </summary>
MaterialData ModelManager::LoadMaterialTemplateFile(const std::string& filePath, const std::string& fileName) {
