...
void SpriteGraphicPipeline::MakeRootSignature() {
	...
	HRESULT hr_ = D3D12SerializeRootSignature(&descriptionRootSignature_,
		D3D_ROOT_SIGNATURE_VERSION_1, &spritePso_.signatureBlob, &spritePso_.errorBlob);
	if (FAILED(hr_)) {
		Log(reinterpret_cast<char*>(spritePso_.errorBlob->GetBufferPointer()));
		assert(false);
	}
	// バイナリを元に生成
	hr_ = DirectXCommon::GetInstance()->GetDevice()->CreateRootSignature(0, spritePso_.signatureBlob->GetBufferPointer(),
		spritePso_.signatureBlob->GetBufferSize(), IID_PPV_ARGS(&spritePso_.rootSignature));
...
void SpriteGraphicPipeline::SetShaderCompile() {
	...
	// Shaderをコンパイルする
	vertexShaderBlob_ = ShaderManager::GetInstance()->GetShaderType().Sprite.VertexBlob;
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = ShaderManager::GetInstance()->GetShaderType().Sprite.PixelBlob;
...
void SpriteGraphicPipeline::CreatePipelineStateObject() {
	...
	graphicsPipelineStateDesc_.pRootSignature = spritePso_.rootSignature; // RootSignature
	...
	HRESULT hr_ = DirectXCommon::GetInstance()->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc_,
		IID_PPV_ARGS(&spritePso_.graphicsPipelineState));
	assert(SUCCEEDED(hr_));
}